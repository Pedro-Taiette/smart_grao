...
React + TypeScript + MUI, com o cliente da API gerado pelo Orval a partir do contrato OpenAPI do
backend.
...
| `npm run api` | **regera o cliente da API** a partir de `../openapi/SmartGrao.WebApi.json` |
...
src/
  ...
  features/
    farms/
      hooks/        useFarms, useSaveFarm, useRemoveFarm, useFarmForm
      components/   FarmCard, FarmFormDialog
      pages/        FarmsPage
      farmSchema.ts validação (zod) + conversão para o payload da API
    fields/
      hooks/        useFields, useSaveField, useRemoveField,
                    useToggleFieldStatus, useFieldForm, useFieldsWorkspace
      components/   FieldMap, FieldPolygon, DrawControl, FieldFormDialog, …
      geo/          conversão GeoJSON ↔ Leaflet
      pages/        FieldsPage
...
**Mensagens de erro vêm de um catálogo único.** O backend nunca manda texto para o usuário: manda um
código estável (`field.overlaps_another`). O `apiErrors.ts` é o outro lado desse contrato e é o único
arquivo com frases em português para falhas.
...
```powershell
# no backend: o build já emite ../openapi/SmartGrao.WebApi.json
dotnet build backend/SmartGrao.slnx
...
Uma mudança incompatível aparece como erro de compilação nos hooks de feature — que é exatamente
onde ela deve doer.
